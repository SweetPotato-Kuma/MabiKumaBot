...
- `/구마` 하나로 아이템 추가/삭제, 목록 확인, 알림 채널 설정, 체크 간격 설정, 가격 확인 지원
- `마나허브`처럼 공백을 빼고 입력해도 `마나 허브` 같은 실제 경매장 아이템명을 찾아 가격 조회
- 아이템 목록을 `data/items.json`에 저장
- 알림 채널을 `/구마` UI에서 설정하고 `data/settings.json`에 저장
...
- `DISCORD_CHANNEL_ID`: 시작 시 미리 지정할 알림 채널 ID. 비워 두고 `/구마` UI에서 지정해도 됩니다.
...
- 아이템 추가
- 목록/삭제: 목록에 표시된 `아이템 X` 버튼으로 항목을 삭제하고, 페이지 버튼으로 이동
- 가격 확인
- 현재 채널을 알림 채널로 설정
- 체크 간격 설정
- 상태 확인
