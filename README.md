...
- 설정/상태: 현재 채널 알림 설정, 알림 기준, 체크 간격, 상태 확인

아이템명을 입력하면 경매장 후보가 있는 경우 선택 메뉴로 실제 아이템명과 자동 분류를 확인할 수 있습니다. 아이템 추가 시에는 Nexon 경매장 매물 존재 여부를 검증한 뒤, 확인된 분류와 관련 검색어를 함께 저장합니다.
